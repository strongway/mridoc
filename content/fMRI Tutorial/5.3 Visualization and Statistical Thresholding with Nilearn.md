---
title: 5.3 Visualization and Statistical Thresholding with Nilearn
draft: false
tags:
  - nilearn
  - visualization
...
The statistics being thresholded here are the per-voxel t/z maps produced by the GLM
([[3.2 General Linear Model]]); this note is where the **multiple-comparisons problem**
raised there gets solved.
...
### 1.1 How FDR works — the Benjamini–Hochberg procedure

FDR is not a single fixed threshold; it is a *procedure* that reads the threshold off your own data. The classic algorithm is **Benjamini & Hochberg (1995)**. Given $m$ voxels, each with a p-value, and a target level $q$ (e.g. 0.05):

1. **Sort** all voxel p-values in ascending order: $p_{(1)} \le p_{(2)} \le \dots \le p_{(m)}$.
2. For each rank $i$, form the **BH line** $\frac{i}{m}\,q$.
3. Find the **largest** rank $k$ for which $p_{(k)} \le \frac{k}{m}\,q$ (the last point where the sorted p-values are still below the line).
4. **Reject** (declare significant) the voxels with the $k$ smallest p-values; the effective threshold is $p_{(k)}$.

**Why the line?** For the $i$-th smallest p-value, $\frac{i}{m}q$ is the value it would take if exactly a $q$-fraction of the first $i$ "discoveries" were false. A voxel therefore survives only when its p-value is small *relative to its rank* — the more voxels that already show small p-values, the more forgiving the cutoff becomes. This is why FDR is **data-adaptive**: a map with strong, widespread signal produces many tiny p-values and gets a **lenient** threshold, whereas a map of pure noise pushes the crossing all the way down to $p_{(1)} \le q/m$ — i.e. it collapses to **Bonferroni**. FDR can never be *more* lenient than the signal in the data justifies.

The plot below runs exactly this procedure on a real group `active − passive` z-map ($m = 269{,}686$ in-brain voxels, $q=0.05$). The sorted p-values (blue) cross the BH line (dashed red) at rank **10,562**, so those 10,562 voxels are declared significant — equivalent to a voxel threshold of $z > 3.10$.

![[fdr_bh.png]]

In Nilearn, `threshold_stats_img(..., height_control='fdr')` implements precisely this Benjamini–Hochberg step; you never sort p-values by hand, but it is worth knowing that the returned z-threshold is simply the $p_{(k)}$ crossing translated back into a z-score.
...
Let’s apply a **voxel-wise uncorrected p<0.001** threshold. For Z-statistics, p<0.001 (one-tailed) corresponds to Z > ~3.09. We’ll use Z > 3.29 (≈ two-tailed p<0.001, since we threshold |Z|) as in many studies, and also remove tiny clusters (cluster size < 10 voxels) to focus on substantive blobs:
...
Classical cluster-wise correction forces you to pick a **cluster-forming threshold** (commonly z > 3.1 ≈ p < 0.001 uncorrected) before any cluster even exists. That choice is arbitrary, and Eklund et al. (2016) showed that the parametric assumptions behind it can substantially inflate the family-wise error rate when the threshold is too lenient. **TFCE** (Smith & Nichols, 2009) sidesteps the problem: at every voxel, it integrates support from neighbouring voxels across all possible thresholds, then assesses significance via permutation. The result combines voxel-level and cluster-level evidence in a single map, with no cluster-forming threshold to defend. It is now standard in FSL's `randomise` and is widely regarded as the current best practice in many subfields.

Nilearn exposes TFCE through `non_parametric_inference`:

```python
from nilearn.glm.second_level import non_parametric_inference

neg_log_pvals = non_parametric_inference(
    second_level_input=cope_imgs,         # list of first-level cope images
    design_matrix=design_matrix,          # one-sample t-test design
    second_level_contrast="intercept",
    n_perm=5000,                          # ≥ 5000 for stable p-values
    two_sided_test=False,
    threshold=None,                       # TFCE handles thresholding
    tfce=True,
    n_jobs=-1,
)
# neg_log_pvals image: voxels with -log10(p_FWE) > 1.3 are p<0.05 FWE-corrected
```

> [!warning]
> Permutation testing is **expensive**. Expect runtimes from several minutes (small mask, modest n) up to many hours (whole brain, large group, many permutations). 5000 permutations is a reasonable minimum for stable corrected p-values around 0.05 — fewer permutations make the p-value estimate noisy precisely at the threshold you care about. Run it overnight on a server, not on your laptop during office hours.

> [!info] which thresholding method should you use?
> - **Uncorrected p<0.001 + cluster extent ≥ 10 voxels** — exploratory only, not for publication. Inflated false-positive rate (Eklund et al. 2016).
> - **FDR (`fdr_bh`)** — controls expected proportion of false positives among rejected voxels. Adaptive: more permissive when there is widespread true signal. Good when you expect many true effects.
> - **Voxel-wise FWE / Bonferroni** — controls probability of *any* false positive across all voxels. Very conservative; only practical for strong, focal effects.
> - **Cluster-wise FWE** (parametric, Nilearn's `cluster_level_inference` or FSL's cluster correction) — controls FWE across spatial clusters. Need to specify a cluster-forming threshold (typically z > 3.1, equivalent to p < 0.001 uncorrected). The most common choice for whole-brain task fMRI.
> - **TFCE** (`non_parametric_inference` with `tfce=True`) — no arbitrary cluster-forming threshold, principled permutation-based FWE. Current best practice when compute allows it.
>
> **Default for a master's project**: cluster-wise FWE p<0.05 with cluster-forming z > 3.1, *or* TFCE with 5000 permutations. Always report which method and threshold you used.
...
<iframe src="/static/media/view_img_interactive.html" width="800" height="300"></iframe>
...
- Smith, S. M., & Nichols, T. E. (2009). Threshold-free cluster enhancement: Addressing problems of smoothing, threshold dependence and localisation in cluster inference. *NeuroImage*, 44(1), 83–98. [doi:10.1016/j.neuroimage.2008.03.061](https://doi.org/10.1016/j.neuroimage.2008.03.061)
- Eklund, A., Nichols, T. E., & Knutsson, H. (2016). Cluster failure: Why fMRI inferences for spatial extent have inflated false-positive rates. *Proceedings of the National Academy of Sciences*, 113(28), 7900–7905. [doi:10.1073/pnas.1602413113](https://doi.org/10.1073/pnas.1602413113)
- Rosenblatt, J. D., Finos, L., Weeda, W. D., Solari, A., & Goeman, J. J. (2018). All-Resolutions Inference for brain imaging. *NeuroImage*, 181, 786–796. [doi:10.1016/j.neuroimage.2018.07.060](https://doi.org/10.1016/j.neuroimage.2018.07.060)
