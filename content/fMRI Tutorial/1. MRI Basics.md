...
$$\omega_0 = \gamma B_0, \qquad f_0 = \frac{\gamma}{2\pi}\,B_0,$$

where $\omega_0$ is the angular precession frequency (in radians per second), $f_0$ is the precession frequency (in Hz), and $\gamma$ is the **gyromagnetic ratio** (a constant for each nuclear species). By convention, the values quoted in MHz/T below correspond to $\gamma/(2\pi)$, i.e. they convert field strength directly into the linear (Hz) frequency.

The gyromagnetic ratio determines how fast a given type of nucleus precesses in a magnetic field. For protons ($^1H$), $\gamma/(2\pi) \approx 42.58\ \mathrm{MHz/T}$, meaning at $B_0=1.5\ \mathrm{T}$ the proton precesses at about 64 MHz and at 3 T about 128 MHz . 
...
![[C12-FF9.gif]]
(Image originally from [radiologykey.com](https://radiologykey.com/magnetic-resonance-basics-magnetic-fields-nuclear-magnetic-characteristics-tissue-contrast-image-acquisition-2/); self-hosted locally to avoid link rot.)
...
> [!note] Where next
> This note covers how the MR *signal* arises. How that signal is turned into a 3D
> *image* (slice-selection, frequency/phase encoding, k-space) is touched on in
> [[2.1 MRI Data Acquisition and Format]]. The signal most relevant to fMRI — the
> $T_2^*$-weighted BOLD contrast — is the subject of the next note:
> **[[1.1 BOLD Signal and HRF]]**.
...
- [MRIQuestions.com](https://mriquestions.com/) — an excellent website with clear explanations.
