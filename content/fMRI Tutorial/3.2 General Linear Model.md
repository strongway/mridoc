...
$$ \mathrm{Var}(\hat\beta) = \mathrm{Var}\big((X^T X)^{-1} X^T \varepsilon\big) = (X^T X)^{-1} X^T \,(\sigma^2 I)\, X (X^T X)^{-1} = \sigma^2 (X^T X)^{-1}, $$

since $(X^T X)^{-1} X^T X (X^T X)^{-1} = (X^T X)^{-1}$.
...
For an **F-test**, which can test _multiple_ linear constraints simultaneously, $\mathbf{c}$ can be a matrix (with each row a contrast vector). For example, if we have $m$ hypotheses to test jointly, we form a $p \times m$ contrast matrix $C$ and test $H_0: C^T \boldsymbol{\beta} = \mathbf{0}_m$. The F-statistic evaluates how much variance is explained by those $m$ linear combinations relative to residual variance. Explicitly,

$$
F = \frac{(C^T \hat{\beta})^T \, [C^T (X^T X)^{-1} C]^{-1} \, (C^T \hat{\beta}) \, / \, m}{\hat{\sigma}^2},
$$

where $m$ is the number of rows of $C$ (i.e., the number of contrasts being tested jointly), and $\hat{\sigma}^2$ is the residual mean square error. The numerator measures the squared length of the contrast estimate $C^T \hat\beta$, whitened by its own covariance, and averaged across the $m$ contrasts.

This generalises the t-test directly: when $m=1$, $F = t^2$, so a one-row F-test is just the square of the corresponding two-sided t-test. F-tests are typically used for **omnibus hypotheses**, e.g. "do these three conditions differ _overall_" or "is there any effect of this factor with multiple levels," where a significant F would prompt further post-hoc t-tests.
...
> [!example] Answer
> Yes, the contrast $c = [1, 1]$ does the job. Under dummy coding with F as baseline, $\beta_0$ is the mean of F and $\beta_1 = \mu_M - \mu_F$, so $c^T \beta = \beta_0 + \beta_1 = \mu_F + (\mu_M - \mu_F) = \mu_M$. The null hypothesis $H_0: c^T \beta = 0$ therefore states that the mean BOLD response under condition M is zero (i.e., M does not differ from implicit baseline). Equivalently, this contrast reconstructs the condition-M cell mean from the dummy-coded coefficients.
#### Effects (Sum) Coding
...
In a two-condition example, an effects-coded design might have an intercept equal to the grand mean of F and M, and a predictor where $\beta_1$ represents the difference between M and F. For instance, coding F as $-0.5$ and M as $+0.5$ (a span of 1) yields an intercept $\beta_0 = (\mu_{F}+\mu_{M})/2$ (mean of both conditions) and $\beta_1 = \mu_{M} - \mu_{F}$ (the full difference). (With $\pm 1$ coding the span is 2, so $\beta_1$ would instead be *half* the difference.) In general, with sum coding, the interpretation of $\beta$ coefficients shifts, but any hypothesis about means can still be tested with an appropriate contrast.
...
> [!example] Answer
> No — adding $\beta_B - \beta_C = 0$ on top of the existing two rows does not add information. Note that
> $$ (\beta_A - \beta_B) - (\beta_A - \beta_C) = \beta_C - \beta_B, $$
> so the proposed contrast $[0, 0, 1, -1]$ is a linear combination of the two rows already in $C$. The rank of $C$ stays at $2$, and the F-statistic and its numerator degrees of freedom $m = \text{rank}(C) = 2$ are unchanged. In general, for a three-level factor only $k-1 = 2$ independent contrasts span the space of mean differences; any third pairwise contrast is redundant.
...
So far every regressor has been an **on/off** boxcar: it assumes every trial of a
condition evokes the *same* response amplitude, captured by a single $\beta$. Often,
though, you expect the response to **scale** with a quantity that varies trial to
trial — reaction time, stimulus intensity, difficulty, subjective value, prediction
error. **Parametric modulation** models exactly that, by adding a second regressor
for the condition.

Given a condition with onsets and a per-trial modulator value $m_i$ (for example,
each trial's reaction time), you build **two** regressors from the same onsets:

1. a **main** regressor — the usual boxcar (all heights = 1), convolved with the HRF;
2. a **modulator** regressor — the same boxcar, but each event's height set to the
   **mean-centred** value $m_i - \bar{m}$, then convolved with the HRF.

Their coefficients answer two separate questions:

- $\beta_{\text{main}}$ — the **average** response to the condition (does the region
  respond at all?);
- $\beta_{\text{mod}}$ — the **slope**: how much the response grows per unit of the
  modulator (does the response track reaction time / value / …?).

Mean-centring the modulator keeps it (approximately) orthogonal to the main
regressor, so the two $\beta$'s are separable and $\beta_{\text{main}}$ retains its
"average response" meaning. In Nilearn this is just an extra `modulation` column in
the events table passed to `make_first_level_design_matrix`; in SPM it is a
parametric modulator attached to a condition.

Parametric modulation is not used in this tutorial's categorical active-vs-passive
task, but it is the standard tool the moment a regressor's *amplitude*, rather than
its mere presence, carries the hypothesis (see also [[3.1 Experimental Design]]).
...
X_no_overlap = simulate_design_matrix(n_vols, onsets_A, onsets_B_no_overlap, tr=2.0)
X_overlap   = simulate_design_matrix(n_vols, onsets_A, onsets_B_overlap, tr=2.0)
# Note: total duration = n_vols * TR = 175 * 2 = 350 s; simulate_design_matrix internally uses total secs
...
Thus, **both** (1) *widely varying predictors* and (2) *low correlation* among predictors contribute to lower design variance and higher statistical power. Usually using a randomized (jittered) inter-trial interval (ITI) results in low correlation among predictors. 
...
### From one contrast to a whole-brain map

Everything above yields a t- (or z-) statistic for **one contrast at one voxel**. A
real analysis fits this same GLM at *every* voxel — on the order of 200,000 of them —
so a single statistical map is 200,000 simultaneous hypothesis tests. Tested at an
uncorrected p&lt;0.05, roughly 10,000 voxels would cross threshold *by chance alone*.
Deciding which voxels to actually believe is the **multiple-comparisons problem**,
and it is solved at the group stage with FWER, FDR, or cluster-based correction —
see [[5.3 Visualization and Statistical Thresholding with Nilearn]].

> One more caveat this note glosses over: fMRI noise is **temporally autocorrelated**,
> so the OLS standard errors and $n-p$ degrees of freedom above are not directly valid
> for a BOLD time series. FSL (FILM) and SPM handle this by **prewhitening** (GLS) —
> which is one reason first-level modelling is done with those tools rather than plain OLS.
...
- Additionally, **Vasishth, Schad, Bürki, & Kliegl (2022)** [_Linear Mixed Models for Linguistics and Psychology: A Comprehensive Introduction_](https://vasishth.github.io/Freq_CogSci/) (Chapter 6, "Contrast coding") offers insight into contrast coding in regression, illustrating how different coding schemes (treatment, sum coding, etc.) affect interpretation but not the underlying testable hypotheses.
