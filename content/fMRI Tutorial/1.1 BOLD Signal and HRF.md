...
> [!note] How big is the mismatch?
> When a region becomes active, cerebral blood flow (CBF) increases by ~30–50 % while the cerebral metabolic rate of oxygen consumption (CMRO₂) rises only ~5–15 %. This mismatch — more oxygen delivered than the tissue actually consumes — is what reduces the local deoxyhemoglobin concentration and produces the positive BOLD signal. The "overcompensation" above is this CBF–CMRO₂ gap, not a vague metaphor.
...
> [!warning] Linearity is an approximation
> The convolution model treats the hemodynamic response as a **linear time-invariant (LTI)** system. This holds to first order — for stimuli spaced $\geq$ ~5 s apart, the BOLD response to two events approximately equals the sum of the individual responses (Boynton et al. 1996). For very brief or closely-spaced events the response saturates and superposition breaks down (Wager et al. 2005). Most task fMRI experiments deliberately jitter inter-stimulus intervals to keep events in the linear regime where this convolution model is valid.
...
> [!example] SPM canonical double-gamma parameters
> $\alpha_1 = 6$, $\beta_1 = 1$, $\alpha_2 = 16$, $\beta_2 = 1$, $c = 1/6$.
> Peak at $t \approx (\alpha_1 - 1)\beta_1 = 5$ s, post-stimulus undershoot at ~15 s. These are SPM's `spm_hrf` defaults. Nilearn's `glover_hrf` (used below, Glover 1999) has the same double-gamma *form* but different fitted parameters — a slightly earlier peak and a shallower undershoot (ratio ~0.35 rather than 1/6) — so the two canonical HRFs are similar but not identical.
...
# Generate Glover HRF at 0.1 s resolution over 32 s
tr = 1.0
oversampling = 10
time_length = 32
glover = glover_hrf(tr=tr, oversampling=oversampling,
                    time_length=time_length, onset=0.0)

# Time axis matched to the HRF samples (dt = tr / oversampling)
time_points = np.arange(glover.size) * (tr / oversampling)

# Plot HRF
plt.plot(time_points, glover)
...
Selecting an appropriate HRF model depends on your experimental design, type of stimulus, and brain region under study. While canonical forms like the Glover HRF offer robust approximations, the true HRF varies in **latency** and **width** across subjects, regions, and ages (Handwerker et al. 2004). The standard way to absorb this variability without abandoning the canonical shape is to add its **temporal and dispersion derivatives** as extra regressors: the temporal derivative accommodates small shifts in peak *timing*, the dispersion derivative small changes in peak *width*. SPM and Nilearn expose these as `hrf_model="glover + derivative"` (and `+ dispersion`). For fully flexible modelling, a **finite impulse response (FIR)** basis estimates the response shape at each time point with no assumed form — powerful, but at the cost of many parameters and lower statistical efficiency.

> [!note] Where next
> Convolving each condition's on/off time course with this HRF is exactly what
> builds a **design-matrix regressor**. That convolution is the entire justification
> for the linear model in the next chapter: **[[3.2 General Linear Model]]**.
...
- Uludağ K, et al. (2009). **An integrative model for neuronal activity-induced signal changes for gradient and spin echo fMRI**. _NeuroImage_, **48**(1): 150-165 . (Advanced reading on the biophysical models of the BOLD signal)
 ...
 - Boynton, G. M., Engel, S. A., Glover, G. H., & Heeger, D. J. (1996). Linear systems analysis of functional magnetic resonance imaging in human V1. _Journal of Neuroscience_, _16_(13), 4207–4221. https://doi.org/10.1523/JNEUROSCI.16-13-04207.1996
 - Wager, T. D., Vazquez, A., Hernandez, L., & Noll, D. C. (2005). Accounting for nonlinear BOLD effects in fMRI: parameter estimates and a model for prediction in rapid event-related studies. _NeuroImage_, _25_(1), 206–218.
