...
export BIDS_DIR="${BIDS_DIR:?set BIDS dataset root, e.g. /path/to/ds000003}"
export DERIVS_DIR="${DERIVS_DIR:-$BIDS_DIR/derivatives}"
export WORK_DIR="${WORK_DIR:-$HOME/tmp/fmriprep_work}"
export FS_LICENSE="${FS_LICENSE:?path to FreeSurfer license.txt}"
export FMRIPREP_VERSION=24.1.1

#1. first load the pinned fmriprep module
module load fmriprep/${FMRIPREP_VERSION}

fmriprep $BIDS_DIR $DERIVS_DIR \
	participant\
...
- **Outputs:**
Your preprocessed images and confound regressors (`*_desc-confounds_timeseries.tsv`) are ready for statistical analysis.
...
At the subject level, we estimate the mean effect for each subject, so we only need an intercept and using DummyContrasts. 
...
>[!Info]
>There are two types of first-level (i.e., `Run` level) `glm`: Nilearn's `FirstLevelModel` (default) and AFNI's `3dREMLfit`. If you want to use the latter, specify `--estimator afni` during the command line `fitlins`. The difference is that Nilearn uses an AR(1) noise model on the residuals (configurable via `noise_model`), while AFNI's `3dREMLfit` fits a voxelwise ARMA(1,1) model to account for auto-correlated time-series noise. 
>
...
Each analysis node has fields defined in `Node`. The `Model` specification MUST be invoked prior to `Contrasts` and `DummyContrasts`. 
...
After defining the model json file, next you can run `fitlins` using singularity. On the lab server, the image path is stored in `$FITLINS_IMG` (point this at the local `fitlins.simg`). Pin a concrete FitLins version (e.g., `0.11.0`) — version pinning matters for reproducibility — see [[Software Versions]].
...
export WORK_DIR="${WORK_DIR:-$HOME/tmp/fitlins_work}"
export FITLINS_VERSION=0.11.0
export FITLINS_IMG="${FITLINS_IMG:-$HOME/containers/fitlins-${FITLINS_VERSION}.simg}"

output="$DERIVS_DIR/fitlins"                       # fitlins output dir
model_file="$BIDS_DIR/code/linmodel.json"          # model json file

# run the model
singularity run --cleanenv \
	$FITLINS_IMG $BIDS_DIR $output \
	dataset \
